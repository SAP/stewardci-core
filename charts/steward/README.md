...
| <code>pipelineRuns.<wbr/>jenkinsfileRunner.<wbr/>terminationMessagePath</code> | (string)<br/> The path of the termination message file in the Jenkinsfile Runner container. For a description see field `terminationMessagePath` of the `container` spec in the Kubernetes API documentation. If not set, the Kubernetes default will be used. | A value as required by the default Jenkinsfile Runner container image. |
...
| <code>pipelineRuns.<wbr/>limitRange</code> | (string)<br/> The limit range to be created in every pipeline run namespace. The value must be a string containing a complete version `v1` of a kind `LimitRange` in YAML format. The `.metadata` section of the manifest can be omitted, as it will be replaced anyway. See the [Kubernetes documentation of limit range][k8s-limitranges] for details about Kubernetes limit range.<br/><br/> Note that steward ensures that all containers of each pod in pipeline run in namespaces get the default CPU and Memory request and limits (if not provided in the pod definition) | All containers of each pod in pipeline run in a single namespace get default CPU request of 0.5 core, default Memory request of 0.5 GiB, default CPU limit of 3 cores and default Memory limit of 3 GiB. |
| <code>pipelineRuns.<wbr/>resourceQuota</code> | (string)<br/> The resource quota to be created in every pipeline run namespace. The value must be a string containing a complete version `v1` of a kind `ResourceQuota` in YAML format. The `.metadata` section of the manifest can be omitted, as it will be replaced anyway. See the [Kubernetes documentation of resource quota][k8s-resourcequotas] for details about Kubernetes resource quota.| empty |
