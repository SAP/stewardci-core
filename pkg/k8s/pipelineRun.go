...
func NewPipelineRun(apiObj *api.PipelineRun, factory ClientFactory, durationObserver DurationObserver) (PipelineRun, error) {
	if factory == nil {
		return &pipelineRun{
			apiObj:           apiObj,
			copied:           false,
...
func (r *pipelineRun) CommitStatus() ([]*api.StateItem, error) {
	...
	defer func(start time.Time) {
		if r.durationObserver != nil {
			elapsed := time.Since(start)
			klog.V(5).Infof("commit of %q took %v, retry: %t", r.String(), elapsed, isRetry)
			r.durationObserver.ObserveDuration(elapsed, isRetry)
		}
	}(time.Now())
