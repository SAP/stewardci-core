...
func Test_NewPipelineRun_IsCopy(t *testing.T) {
	...
	// EXERCISE
	examinee, err := NewPipelineRun(ctx, run, factory)

	// VERIFY
	assert.NilError(t, err)
	examinee.UpdateResult(ctx, api.ResultSuccess, metav1.Now())
...
func Test_pipelineRun_StoreErrorAsMessage(t *testing.T) {
	...
	// EXERCISE
	examinee.StoreErrorAsMessage(ctx, errorToStore, message)
	examinee.CommitStatus(ctx)
...
func Test_pipelineRun_InitState(t *testing.T) {
	...
	examinee, err := NewPipelineRun(ctx, pipelineRun, factory)
	assert.NilError(t, err)

	// EXERCISE
	resultErr := examinee.InitState(ctx)
...
func Test_pipelineRun_InitState_ReturnsErrorIfCalledMultipleTimes(t *testing.T) {
	...
	for _, oldState := range []api.State{
		...
		t.Run(string(oldState), func(t *testing.T) {
			...
			resultErr = examinee.UpdateState(ctx, oldState, metav1.Now())
			assert.NilError(t, resultErr)
...
func Test_pipelineRun_UpdateState_HasAutomaticInitialization(t *testing.T) {
	...
	// EXERCISE
	resultErr := examinee.UpdateState(ctx, api.StatePreparing, metav1.Now())
	assert.NilError(t, resultErr)
	results, resultErr := examinee.CommitStatus(ctx)
...
func Test_pipelineRun_UpdateState_AfterSecondCall(t *testing.T) {
	...
	err = examinee.InitState(ctx)
	assert.NilError(t, err)
	err = examinee.UpdateState(ctx, api.StatePreparing, metav1.Now()) // first call
	assert.NilError(t, err)
	factory.Sleep("let time elapse to check timestamps afterwards")

	// EXERCISE
	resultErr := examinee.UpdateState(ctx, api.StateRunning, metav1.Now()) // second call
	assert.NilError(t, resultErr)
	results, resultErr := examinee.CommitStatus(ctx)
...
func Test_pipelineRun_UpdateStateToFinished_HistoryIfUpdateStateCalledBefore(t *testing.T) {
	...
	err = examinee.InitState(ctx)
	assert.NilError(t, err)
	err = examinee.UpdateState(ctx, api.StatePreparing, metav1.Now()) // called before
	assert.NilError(t, err)
	factory.Sleep("let time elapse to check timestamps afterwards")

	// EXERCISE
	examinee.UpdateState(ctx, api.StateFinished, metav1.Now())
	_, err = examinee.CommitStatus(ctx)
	assert.NilError(t, err)
...
func Test_pipelineRun_GetValidatedJenkinsfileRepoServerURL_CorrectURLs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.url, func(t *testing.T) {
			...
			r, err := NewPipelineRun(ctx, run, nil)
			assert.NilError(t, err)
...
func Test_pipelineRun_UpdateState_PropagatesError(t *testing.T) {
	...
	// EXCERCISE
	examinee.UpdateState(ctx, api.StateWaiting, metav1.Now())
	_, err = examinee.CommitStatus(ctx)
...
func Test_pipelineRun_CommitStatus_RetriesOnConflict(t *testing.T) {
	...
	err = examinee.InitState(ctx)
	assert.NilError(t, err)
	resultErr := examinee.UpdateState(ctx, api.StateWaiting, metav1.Now())
	assert.NilError(t, resultErr)

	// EXCERCISE
	_, resultErr = examinee.CommitStatus(ctx)
...
func Test_pipelineRun_CommitStatus_PanicsIfNoClientFactory(t *testing.T) {
	...
	ctx := context.Background()
	run := newPipelineRunWithEmptySpec(ns1, run1)
	examinee, err := NewPipelineRun(ctx, run, nil /* client factory */)
	assert.NilError(t, err)
