...
| `spec.timeout` | (string,optional) The timeout value specified for a whole steward pipeline run (overarching all states). The duration string format of composed of whole numbers, each with a unit suffix, such as "300m", "15h" or "2h45m". Valid time units are "s", "m" and "h". |
